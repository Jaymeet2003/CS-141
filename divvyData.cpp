/* 
    ...
    Coordinates for Home I would like to live in:  41.8645  -87.629
*/
...
vector<string> parseString(string x) {
    string temp = "";
    vector<string> res;
...
Ride getNewRide(string x) {
    Ride ride;
    vector<string> data = parseString(x);
...
void displayOverAllTripInfo(vector<Ride> cleanRides) {
...
void casualMemberRiderPercent(vector<Ride> cleanRides) {
...
void displayNumberOfChars(char c, int count) {
...
void calculationPerHour(vector<Ride> cleanRides, int choice) {
    ...
    if (choice == 1) {
        ...
        for (int i = 0 ; i <= 23; i++) {
            if ( i < 10) {
                cout << "    " << i << ": ";
                displayNumberOfChars('@', (weekday[i] * 1.0/largestHour) * 50);
                cout << "      ";
                displayNumberOfChars('+', (weekend[i] * 1.0/largestHour) * 50);
            }
            else {
                cout << "   " << i << ": ";
                displayNumberOfChars('@', (weekday[i] * 1.0/largestHour) * 50);
...
int main() {
    ...
    while(true) {
        ...
        if (choice == 2) {
            displayOverAllTripInfo(cleanRides);
        }

        if (choice == 3) {
            casualMemberRiderPercent(cleanRides);
        }
