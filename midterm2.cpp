...
// Constructors (default, fully qualified, copy)
    ...
    Meme( const Meme & otherMeme) {
        ...
        this->howFunny = otherMeme.howFunny;
...
// Getters and setters
    ...
    void setHowFunny( double thehowFunny) { howFunny = thehowFunny; }

    string getUrl()   { return url;   }

    double getHowFunny() { return howFunny; }

// Utility functions

void display() {

        cout << url << " how funny rating " << howFunny << endl;
    ...
    private:
        ...
        int howFunny;   // 1 is low, 5 is high
...

{
    ...
    for( int i=0; i<allMemes.size(); i++) {
        ...
        int theFunniestMemeRating = allMemes.at( i).getHowFunny();

        if( theFunniestMemeRating > funniestMeme) {


            funniestMeme = theFunniestMemeRating;

            indexOfFunniestMeme = i;
...
int main()
{
    ...
    switch(testNumber){
        ...
        case 3:
        {
            ...
            if (m3.getHowFunny() == m4.getHowFunny() && (m3.getUrl()).compare(m4.getUrl()) == 0) {
